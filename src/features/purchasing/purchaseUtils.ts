...
export function buildPurchaseParams(
  ...
  const keys: Array<keyof PurchaseFilters> = [
    "branch_id",
    "supplier_id",
    "status",
    "from",
...
export function canOrderPurchaseOrder(
  role: UserRole | undefined,
  status: string,
) {
  return role === "ADMIN" && status.toUpperCase() === "DRAFT";
}
export function canViewPurchaseCosts(role?: UserRole) {
  return Boolean(role && costRoles.includes(role));
}
export function receiveLineError(qty: string, outstanding: string) {
  ...
  if (requested > available)
    return `Cannot receive more than the outstanding quantity of ${outstanding}.`;
  return undefined;
}
export function buildReceivePayload(
  receivedDate: string,
  lines: Array<{ line_id: string; quantity: string }>,
): ReceivePayload {
  const normalized = lines
    .map(({ line_id, quantity }) => ({
      line_id: Number(line_id),
      received_qty: Number(quantity),
  ...
  if (
    normalized.some(
      (line) =>
        !Number.isInteger(line.line_id) || !Number.isFinite(line.received_qty),
    )
  ) {
    throw new Error(
      "Purchase order receipt contains an invalid line or quantity.",
    );
  }
  if (!normalized.length) {
    throw new Error("Enter a quantity greater than zero for at least one line.");
  }
  return { lines: normalized };
}
export function isReceiveable(status: string) {
  return !["CANCELLED", "RECEIVED", "CLOSED", "DRAFT"].includes(
    status.toUpperCase(),
  );
}
...
export function payableMoney(value?: string | number | null): string | number {
  return value ?? "0.00";
}
export function payableRows(report?: Partial<PayablesReport>): PayableRow[] {
  return report?.rows || [];
}

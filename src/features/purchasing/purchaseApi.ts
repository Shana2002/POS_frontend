...
export const purchaseKeys = {
  ...
  payables: (filters: {
    ...
    supplier_id?: string;
    as_of?: string;
  }) => ["payables", "outstanding", filters] as const,
};

export function usePurchaseOrders(filters: PurchaseFilters) {
  return useQuery({
    queryKey: purchaseKeys.list(filters),
    queryFn: async () => {
      const result = await request<PurchaseOrder[]>({
        method: "GET",
        url: "/purchase-orders",
...
export function usePurchaseOrder(id: string, branchId?: string) {
  return useQuery({
    queryKey: purchaseKeys.detail(id),
    queryFn: async () =>
      (
        await request<PurchaseOrder>({
          method: "GET",
...
export function useOutstandingPayables(filters: {
  ...
  return useQuery({
    queryKey: purchaseKeys.payables(filters),
    queryFn: async () =>
      (
        await request<PayablesReport>({
          method: "GET",
...
export function usePurchaseMutations() {
  ...
  const invalidateOrder = (id?: string) => {
    void client.invalidateQueries({ queryKey: purchaseKeys.all() });
    if (id)
      void client.invalidateQueries({ queryKey: purchaseKeys.detail(id) });
  };
  const create = useMutation({
    ...
    onSuccess: (po) => invalidateOrder(po.id),
  });
  ...
  const receive = useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: ReceivePayload }) =>
      request<ReceiptResult>({
        method: "POST",
        url: `/purchase-orders/${id}/receive`,
        data: payload,
      }).then((r) => r.data),
    onSuccess: (result) => {
      invalidateOrder(result.purchase_order.id);
  ...
  const pay = useMutation({
    ...
    onSuccess: (result) => {
      ...
      void client.invalidateQueries({
        queryKey: purchaseKeys.payments(result.purchase_order.id),
      });
      invalidateFinancialQueries(client, ["payables", "reports"]);
    },
  });

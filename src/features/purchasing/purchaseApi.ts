...
export function usePurchaseMutations() {
  ...
  const receive = useMutation({ mutationFn: ({ id, payload }: { id: string; payload: ReceivePayload }) => request<ReceiptResult>({ method: 'POST', url: `/purchase-orders/${id}/receive`, data: payload }).then((r) => r.data), onSuccess: (result) => { invalidateOrder(result.purchase_order.id); void client.invalidateQueries({ queryKey: ['stock'] }); void client.invalidateQueries({ queryKey: ['payables'] }); void client.invalidateQueries({ queryKey: ['products', 'movement'] }) } })
  const pay = useMutation({ mutationFn: ({ id, payload }: { id: string; payload: PaymentPayload }) => request<PaymentResult>({ method: 'POST', url: `/purchase-orders/${id}/payments`, data: payload }).then((r) => r.data), onSuccess: (result) => { invalidateOrder(result.purchase_order.id); void client.invalidateQueries({ queryKey: purchaseKeys.payments(result.purchase_order.id) }); void client.invalidateQueries({ queryKey: ['payables'] }) } })

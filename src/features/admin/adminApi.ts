...
export function useUser(id: string) {
  return useQuery({
    ...
    queryFn: async () =>
      (await request<User>({ method: "GET", url: `/users/${id}` })).data,
    enabled: Boolean(id),
  });
}
export function useBranches(filters: ListFilters) {
  return useQuery({
    queryKey: adminKeys.branches(filters),
    queryFn: async () => {
      const result = await request<Branch[]>({
        method: "GET",
        url: "/branches",
        params: buildListParams(filters),
      });
...
export function useBranch(id: string) {
  return useQuery({
    queryKey: adminKeys.branch(id),
    queryFn: async () =>
      (await request<Branch>({ method: "GET", url: `/branches/${id}` })).data,
    enabled: Boolean(id),
  });
}
export function useSettings() {
  return useQuery({
    ...
    queryFn: async () =>
      (await request<Setting[]>({ method: "GET", url: "/settings" })).data,
  });
}
export function useSetting(key: string) {
  return useQuery({
    ...
    queryFn: async () =>
      (await request<Setting>({ method: "GET", url: `/settings/${key}` })).data,
    enabled: Boolean(key),
  });
}
...
export function useAdminMutations() {
  ...
  const createUser = useMutation({
    mutationFn: (payload: UserPayload) =>
      request<User>({ method: "POST", url: "/users", data: payload }).then(
        (result) => result.data,
      ),
  ...
  const updateUser = useMutation({
    mutationFn: ({
      ...
      request<User>({ method: "PUT", url: `/users/${id}`, data: payload }).then(
        (result) => result.data,
      ),
    onSuccess: invalidateUsers,
  });
  ...
  const createBranch = useMutation({
    mutationFn: (payload: BranchPayload) =>
      request<Branch>({ method: "POST", url: "/branches", data: payload }).then(
        (result) => result.data,
      ),
  ...
  const deactivateBranch = useMutation({
    mutationFn: (id: string) =>
      request<Branch>({ method: "DELETE", url: `/branches/${id}` }).then(
        (result) => result.data,
      ),
    onSuccess: invalidateBranches,
  });

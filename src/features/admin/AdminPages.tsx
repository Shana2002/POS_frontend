...
function useListState() {
  ...
  const set = (values: Record<string, string | number | undefined>) => {
  ...
  return { params, page, perPage, search, set };
}
  ...
  return (
    <div className="admin-page">
      <AdminHeader
        ...
        title="Users"
        description="Manage account access and active status."
      ...
      <div className="admin-toolbar">
        <input
          ...
          value={list.search}
          onChange={(event) =>
            list.set({ search: event.target.value, page: 1 })
          }
        />
        <button
      ...
      {query.isPending ? (
        ...
        <EmptyState title="No users found">
          Try a different search or create the first account.
        </EmptyState>
      ) : (
      ...
      {(creating || editing) && (
        <UserForm
          user={editing}
          pending={
            mutations.createUser.isPending || mutations.updateUser.isPending
          }
          error={mutations.createUser.error || mutations.updateUser.error}
          onCancel={() => {
            setCreating(false);
            setEditing(null);
          }}
          onSubmit={async (payload) => {
            ...
            else await mutations.createUser.mutateAsync(payload);
            setCreating(false);
            setEditing(null);
          }}
        />
      ...
      {confirm && (
        <ConfirmationDialog
          open
          title={`${confirm.is_active ? "Deactivate" : "Activate"} ${confirm.full_name}?`}
          message={getAdminActionMessage("user", confirm.is_active)}
          pending={mutations.setUserStatus.isPending}
          onCancel={() => setConfirm(null)}
          onConfirm={async () => {
            await mutations.setUserStatus.mutateAsync({
              id: confirm.id,
    ...
    <div className="admin-page">
      <AdminHeader
        ...
        description={
          admin
            ...
            : "View branch records available to your account."
        }
        action={
          admin ? (
            <button onClick={() => setCreating(true)}>Create branch</button>
          ) : undefined
        }
      />
      <div className="admin-toolbar">
            ...
            <tbody>
              {rows.map((branch) => (
                <tr key={branch.id}>
                  ...
                  <td>{branch.is_warehouse ? "Yes" : "No"}</td>
                  <td>
      ...
      {admin && (creating || editing) && (
        <BranchForm
          branch={editing}
          pending={
            mutations.createBranch.isPending || mutations.updateBranch.isPending
          }
          error={mutations.createBranch.error || mutations.updateBranch.error}
          onCancel={() => {
            setCreating(false);
            setEditing(null);
          }}
          onSubmit={async (payload) => {
            ...
            else await mutations.createBranch.mutateAsync(payload);
            setCreating(false);
            setEditing(null);
          }}
        />
      ...
      {admin && confirm && (
        <ConfirmationDialog
          ...
          message={getAdminActionMessage("branch", true)}
          pending={mutations.deactivateBranch.isPending}
          onCancel={() => setConfirm(null)}
          onConfirm={async () => {
            await mutations.deactivateBranch.mutateAsync(confirm.id);
            setConfirm(null);
          }}
        />
      )}
...
function BranchForm({
  ...
  const set = (key: keyof BranchPayload, value: string | boolean) =>
    setForm((current) => ({ ...current, [key]: value }));
  async function submit(event: FormEvent) {
    event.preventDefault();
    await onSubmit(form);
  ...
  return (
    <Modal title={branch ? "Edit branch" : "Create branch"} onClose={onCancel}>
      <form className="admin-form" onSubmit={submit}>
        <FormInput
          label="Code"
          value={form.code}
        ...
        <FormInput
          ...
          onChange={(value) => set("invoice_prefix", value)}
        />
        <label className="check-field">
          <input
            type="checkbox"
            checked={Boolean(form.is_warehouse)}
  ...
  return (
    <div className="admin-page">
      <AdminHeader
        ...
        title="Settings"
        description="Review and update backend-managed application settings."
      />
      {query.isPending ? (
        <LoadingRows />
      ) : query.isError ? (
        ...
        <EmptyState title="No settings returned">
          The backend has not configured any editable settings.
        </EmptyState>
      ) : (
          ...
          {query.data.map((setting) => (
            <article className="setting-row" key={setting.key}>
              <div>
                ...
                <span>
                  {setting.data_type} · Updated{" "}
                  {setting.updated_at || "not available"}
                </span>
              </div>
              <div>
      ...
      {selected && (
        <SettingForm
          setting={selected}
          pending={mutations.updateSetting.isPending}
          error={mutations.updateSetting.error}
          onCancel={() => setSelected(null)}
          onSubmit={async (value) => {
            await mutations.updateSetting.mutateAsync({
              key: selected.key,
              value,
...
function SettingForm({
  ...
  const inputType = getSettingInputType(setting.data_type);
  return (
    <Modal title={`Edit ${setting.key}`} onClose={onCancel}>
      <form

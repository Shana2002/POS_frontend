...
export function useInvoiceMutations() {
  ...
  const refreshStock = () => invalidateFinancialQueries(client, ['stock', 'products', 'receivables', 'reports'])
  ...
  const cancel = useMutation({ mutationFn: ({ id, reason }: { id: string; reason: string }) => request<CancelResult>({ method: 'POST', url: `/invoices/${id}/cancel`, data: { reason } }).then((r) => r.data), onSuccess: (result) => { refresh(result.invoice); refreshStock() } })

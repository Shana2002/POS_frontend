...
export function useInvoice(id: string, branchId?: string) {
  return useQuery({
...
export function useInvoiceMutations() {
  ...
  const addLine = useMutation({
    mutationFn: ({ id, line }: { id: string; line: InvoiceLineInput }) =>
      request<Invoice>({
        method: "POST",
  ...
  const deliver = useMutation({
    mutationFn: ({
      ...
      request<Invoice>({
        method: "PATCH",
        url: `/invoices/${id}/lines/${lineId}/delivery`,
        data: payload,
      }).then((r) => r.data),
    onSuccess: refresh,
  });
...
export function serializeInvoiceLineMutation(
  key: string,
  operation: () => Promise<Invoice>,
) {
  const previous = lineQueues.get(key)?.catch(() => undefined);
  const next = (previous ? previous.then(operation) : operation()).finally(
    () => {

...
function Header({
  ...
  return (
    <>
      <div className="page-heading invoice-header">
        <div>
          <span className="section-kicker">Sales operations</span>
          <h1>{title}</h1>
          <p>{description}</p>
        </div>
        {action}
      </div>
      <nav className="invoice-tabs" aria-label="Invoice views">
        <Link to="/pos">Cashier workspace</Link>
...
function Status({ status }: { status: string }) {
  return (
    <span
      className={`invoice-status ${status.toLowerCase().replaceAll("_", "-")}`}
    >
...
function BranchField({
  ...
  const branch = query.data?.rows.find((row) => row.id === value);
  return (
    <label>
      Branch
      {locked ? (
        <input
          aria-label="Assigned sales branch"
          value={branch ? `${branch.code} · ${branch.name}` : value}
          readOnly
        />
      ) : (
        <select
          aria-label="Invoice branch"
          value={value}
          onChange={(e) => onChange(e.target.value)}
          required={!all}
        >
          {all && <option value="">All branches</option>}
          {!all && <option value="">Select branch</option>}
...
export function InvoicesPage() {
  const { params, setParams, scope } = useInvoiceScope();
  const filters = {
    branch_id: scope.branchId,
    customer_id: params.get("customer_id") || "",
    status: params.get("status") || "",
    sales_rep_id: params.get("sales_rep_id") || "",
  ...
  const set = (key: string, value: string) => updateUrl(setParams, key, value);
  const rows = query.data?.rows || [];
  return (
    <div className="invoice-page">
      <Header
        title="Invoices"
        description="Draft, issue, deliver, cancel, and print sales documents."
        action={
          <Link className="primary-link" to="/invoices/new">
            New invoice
          </Link>
        }
      />
      <div className="invoice-toolbar">
        ...
        <label>
          Customer
          <select
            aria-label="Filter invoice customer"
        ...
        <label>
          ...
          >
            ...
            <option>PARTIALLY_DELIVERED</option>
              ...
              {rows.map((invoice) => (
                <tr key={invoice.id}>
                  <td>
                    ...
                    {freeIssue(invoice) && (
                      <small className="free-issue">Free issue</small>
                    )}
                  </td>
                  <td>
                  ...
                  <td>{formatMoney(invoice.net_amount)}</td>
...
export function InvoiceEditorPage() {
  ...
  const auth = useAuth();
  const existing = useInvoice(id, auth.user?.branch_id || undefined);
  const mutations = useInvoiceMutations();
  const editing = Boolean(id);
  if (editing && existing.isPending) return <Loading />;
  if (editing && (!existing.data || existing.isError))
    return <ErrorState error={existing.error} />;
  if (existing.data && !canEditInvoice(existing.data.status))
    return (
      <div className="invoice-page">
        <Header
          title="Invoice is read only"
          description="Issued invoices cannot be edited as drafts."
        />
  ...
  return (
    <InvoiceForm
      ...
      assignedBranch={auth.user?.branch_id || ""}
      role={auth.user!.role}
...
function InvoiceForm({
  ...
  role: Parameters<typeof invoiceScope>[0];
  pending: boolean;
  error: unknown;
  ...
  const [branch, setBranch] = useState(initial?.branch_id || scope.branchId);
  ...
  const [salesRep, setSalesRep] = useState(initial?.sales_rep_id || "");
  ...
  const duplicate = hasDuplicateInvoiceProducts(lines);
  const update = (index: number, key: keyof InvoiceLineInput, value: string) =>
    setLines((current) =>
      current.map((line, i) =>
  ...
  const submit = (event: FormEvent) => {
    ...
    if (!customer || !branch || duplicate || normalized.length !== lines.length)
      return;
    void onSubmit({
  ...
  return (
    <div className="invoice-page wide">
      <Link
        className="back-link"
        to={initial ? `/invoices/${initial.id}` : "/invoices"}
      >
        ← Back to invoices
      </Link>
      <Header
        title={initial ? `Edit ${initial.invoice_no}` : "New invoice"}
        description="Save a server-calculated draft before issuing stock."
      />
      <form className="invoice-editor" onSubmit={submit}>
        <section className="invoice-fields">
          <label>
            Customer
            <select
          ...
          <label>
            Sales rep ID
            <input
              aria-label="Invoice sales representative"
              value={salesRep}
        ...
        <section className="invoice-line-editor">
          <div className="invoice-edit-line heading">
            <span>Product</span>
            <span>Quantity</span>
          ...
          {lines.map((line, index) => (
            <div
              className={`invoice-edit-line ${lines.filter((item) => item.product_id && item.product_id === line.product_id).length > 1 ? "invalid" : ""}`}
              key={index}
            >
              <select
                aria-label={`Invoice line ${index + 1} product`}
              ...
              <input
                aria-label={`Invoice line ${index + 1} quantity`}
                inputMode="decimal"
                value={line.qty}
              ...
              />
              <input
                aria-label={`Invoice line ${index + 1} price`}
                inputMode="decimal"
        ...
        <div className="invoice-editor-actions">
          ...
          <button disabled={pending || duplicate}>
            {pending ? "Saving draft..." : "Save draft invoice"}
          </button>
        </div>
        {Boolean(error) && <ErrorState error={error} />}
        {discountApprovalMessage(error) && (
...
export function InvoiceDetailPage() {
  const { id = "" } = useParams();
  const auth = useAuth();
  const query = useInvoice(id, auth.user?.branch_id || undefined);
  const mutations = useInvoiceMutations();
  ...
  if (!invoice || query.isError) return <ErrorState error={query.error} />;
  const loadPdf = async (download: boolean) => {
    try {
      const file = await getInvoicePdf(id);
      if (download) downloadBlob(file.blob, file.filename);
      else {
        if (pdf) URL.revokeObjectURL(pdf);
        setPdf(URL.createObjectURL(file.blob));
      }
    } catch (error) {
      setPdfError(error);
    }
  ...
  return (
    <div className="invoice-page wide">
      ...
      <Header
        ...
        description={`${invoice.customer_code} · ${invoice.customer_name}`}
        action={
          <div className="header-actions">
            {canEditInvoice(invoice.status) && (
              <Link className="primary-link" to={`/invoices/${id}/edit`}>
                Edit draft
              </Link>
            )}
            {canEditInvoice(invoice.status) && (
              <button onClick={() => setIssueConfirm(true)}>
                Issue invoice
              </button>
            )}
            {canDeliverInvoice(invoice.status) && (
            ...
            <button
              className="secondary-button"
              onClick={() => void loadPdf(false)}
            >
              Preview PDF
            </button>
            <button
              className="secondary-button"
              onClick={() => void loadPdf(true)}
            >
              Download PDF
            </button>
          </div>
      ...
      />
      {freeIssue(invoice) && (
        <div className="free-issue-banner">
          ! Free issue — zero-revenue stock movement
        </div>
      )}
      <section className="invoice-summary">
        ...
        <article>
          <span>Gross</span>
          <strong>{formatMoney(invoice.gross_amount)}</strong>
        </article>
        <article>
          <span>Discount</span>
      ...
      {invoice.cancel_reason && (
        <div className="cancellation-result">
          <strong>Cancellation posted with reversal movements</strong>
          <span>{invoice.cancel_reason}</span>
        </div>
      )}
      {Boolean(pdfError) && (
        <ErrorState error={pdfError} title="Unable to load invoice PDF" />
      )}
      {pdf && (
        <section className="pdf-preview">
          ...
          <iframe title={`PDF preview ${invoice.invoice_no}`} src={pdf} />
        </section>
      )}
      <ConfirmationDialog
        open={issueConfirm}
        title="Issue invoice"
        message="Issuing this invoice posts stock movements and makes the draft read-only. Continue?"
        pending={mutations.issue.isPending}
        onCancel={() => setIssueConfirm(false)}
        onConfirm={() =>
          void mutations.issue
            .mutateAsync(id)
            .then(() => setIssueConfirm(false))
        }
      />
      {cancelOpen && (
        <ReasonDialog
          title="Cancel invoice"
          consequence="Cancellation posts reversal stock movements. The invoice remains in history."
          pending={mutations.cancel.isPending}
      ...
      {deliveryOpen && (
        <DeliveryDialog
          invoice={invoice}
          pending={mutations.deliver.isPending}
          error={mutations.deliver.error}
          onClose={() => setDeliveryOpen(false)}
          onSubmit={(lineId, delivered_qty, delivery_date) =>
            void mutations.deliver
              .mutateAsync({
                id,
                lineId,
                payload: {
    ...
    <Modal title={title} onClose={onClose}>
      ...
      >
        ...
        <div className="modal-actions">
          ...
          >
            {pending ? "Cancelling..." : "Cancel and reverse"}
          </button>
        </div>
        {Boolean(error) && <ErrorState error={error} />}
      </form>
...
function DeliveryDialog({
  ...
  error: unknown;
  onClose: () => void;
  onSubmit: (lineId: string, qty: string, date: string) => void;
}) {
  const eligible = invoice.lines.filter((line) => line.outstanding_qty !== "0");
  const [lineId, setLineId] = useState(eligible[0]?.id || "");
  const [qty, setQty] = useState("");
  const [date, setDate] = useState("");
  return (
    <Modal title="Update delivery" onClose={onClose}>
      <form
        className="delivery-form"
        onSubmit={(e) => {
          e.preventDefault();
          onSubmit(lineId, qty, date);
        }}
      >
        <label>
          Invoice line
          <select
            aria-label="Delivery invoice line"
            value={lineId}
    ...
    <div className="modal-backdrop">
      ...
      >
        <div className="modal-heading">
          <h2 id="invoice-modal-title">{title}</h2>
          <button
...
export function PosPage() {
  ...
  const branchLocked = ["BRANCH_MANAGER", "SALES_REP"].includes(
    auth.user!.role,
  );
  ...
  useEffect(() => {
    const warn = (event: BeforeUnloadEvent) => {
      if (draft?.status === "DRAFT" && draft.lines.length)
        event.preventDefault();
    };
  ...
  const ensureDraft = async (productId: string) => {
    if (!customer || !branchId)
      throw new Error(
        "Select a customer and assigned branch before adding products.",
      );
    if (draft)
      return mutations.addLine.mutateAsync({
        id: draft.id,
        line: { product_id: productId, qty: "1" },
      });
  ...
  const add = async (productId: string) => {
    ...
    try {
      ...
      const next = existing
        ? await serializeInvoiceLineMutation(
            `${draft!.id}:${existing.id}`,
            () =>
              mutations.updateLine.mutateAsync({
                id: draft!.id,
                lineId: existing.id,
                line: { qty: String(Number(existing.qty) + 1) },
              }),
          )
    ...
    <div className="pos-workspace">
      <section className="pos-catalogue">
        <header className="pos-heading">
          <div>
            <span className="section-kicker">Cashier workspace</span>
            <h1>Build a sales draft</h1>
            {branchLocked ? (
              <p>
                {branchId
                  ? `Assigned branch ${branchId}`
                  : "A branch assignment is required."}
              </p>
            ) : (
        ...
        <div className="pos-result-count">
          {products.data?.rows.length || 0} products
        </div>
        {!navigator.onLine && (
          <div className="offline-state">
            ! Offline — product data may be stale and checkout is unavailable.
          </div>
        )}
        {products.isPending ? (
          <Loading />
          ...
          <EmptyState title="No sale products">
          ...
          <div className="pos-product-grid">
            {products.data.rows.map((product) => {
              const unavailable =
                !product.unit_price || product.is_active === false;
              return (
                <article
                ...
                >
                  ...
                  <div>
                    ...
                    {product.unit_price && (
                      <b>{formatMoney(product.unit_price)}</b>
                    )}
                    <span
                      className={
                        unavailable
                    ...
                    >
                      {busyLine === product.id ? "Adding..." : "Add to order"}
                    </button>
                  </div>
                </article>
              );
      ...
      <aside className={`pos-cart ${cartOpen ? "open" : ""}`}>
        <header>
          <div>
            ...
            <h2>{draft?.invoice_no || "New draft"}</h2>
          </div>
          <button
        ...
        <div className="cart-lines">
          {!cart.length ? (
            <EmptyState title="Cart is empty">
              Select a customer, then add a product.
            </EmptyState>
          ) : (
            cart.map((line) => (
              <article key={line.id}>
                <div>
                  ...
                  <small>{formatMoney(line.unit_price)} each</small>
                  {lineError(details, line.product_id) && (
        ...
        <section className="cart-totals">
          <div>
            <span>Gross</span>
            <strong>
              {draft ? formatMoney(draft.gross_amount) : "LKR 0.00"}
            </strong>
          </div>
          <div>
            <span>Discount</span>
          ...
          {draft && freeIssue(draft) && (
            <div className="free-issue-banner">! Free issue · zero revenue</div>
          )}
        </section>
        {Boolean(apiError) && <ErrorState error={apiError} />}
        {discountApprovalMessage(apiError) && (
          <div className="discount-warning">
            ! {discountApprovalMessage(apiError)}
          </div>
        )}
        ...
        >
          {mutations.issue.isPending ? "Issuing..." : "Issue draft invoice"}
        </button>
        <small className="payment-separation">
          Payment is recorded separately after invoice issue.
        </small>
      </aside>
      <ConfirmationDialog
        open={issueConfirm}
